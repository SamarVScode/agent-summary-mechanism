...
export function useGasSubmit() {
  ...
  async function submit({ date, agentName, casperId, totalCount, completedCount, imageBase64, imageName, file, fileHash }) {
    ...
    try {
      ...
      // 4. Insert a new record into the submissions table
      ...
      if (!dbResponse.ok) {
        ...
        throw new Error(`Database record creation/update failed: ${errorText || dbResponse.statusText}`);
      }

      const resObj = { success: true, imageUrl: publicImageUrl };
      setResult(resObj);
      return resObj;
    } catch (err) {
      const errObj = { success: false, error: err.message || "Network error" };
      setResult(errObj);
      return errObj;
    } finally {
      setSubmitting(false);
